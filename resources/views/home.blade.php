<x-app-layout>
    {{-- Custom Styles (Keep local for now, could be moved to global CSS) --}}
    @push('styles')
    <style>
        .mobile-card {
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950 pb-24 md:pb-12 font-sans selection:bg-blue-500 selection:text-white overflow-x-hidden">
                ...
                <div class="flex items-center gap-4 animate-slide-up">
                    ...
                    <div>
                        <h2 class="text-white text-xl md:text-3xl font-bold max-w-[200px] md:max-w-md leading-tight">
                            <span class="bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-200">{{ Auth::user()->name }}</span>
                        </h2>
                         <h4 class="text-blue-200 text-sm md:text-lg font-semibold max-w-[200px] md:max-w-md leading-tight mt-1">
                            <span class="bg-clip-text text-white bg-gradient-to-r from-blue-100 to-blue-300">{{ Auth::user()->education?->name ?? 'Siswa' }}</span>
                        </h4>
                    </div>
                </div>
                     ...
                     @if($attendanceToday)
                        <div class="flex items-center gap-4 bg-white/10 backdrop-blur-md p-4 rounded-2xl border border-white/20 shadow-xl">
                                ...
                                <div>
                                    ...
                                    <div class="flex gap-3 mt-0.5">
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-white font-mono text-xs">{{ $attendanceToday->time_in ? \Carbon\Carbon::parse($attendanceToday->time_in)->format('H:i') : '-' }}</span>
                                        </div>
                                        <div class="w-px h-6 bg-white/20"></div>
                                        <div class="flex flex-col">
                                            <span class="text-[10px] text-blue-200 uppercase font-bold tracking-wider">Keluar</span>
                                            <span class="text-white font-mono text-xs">{{ $attendanceToday->time_out ? \Carbon\Carbon::parse($attendanceToday->time_out)->format('H:i') : '-' }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            @if(!$attendanceToday->time_out)
                                <a href="{{ route('face-attendance.index') }}" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold text-xs shadow-lg transition-all flex items-center gap-2 group">
                            ...
                            @else
                                <div class="flex flex-col items-end gap-1">
                                    ...
                                    <span class="text-[10px] text-blue-200 font-mono">Durasi: {{ $attendanceToday->duration }}</span>
                                </div>
                            @endif
                        </div>
                     @else
                        <div class="flex items-center gap-3 bg-white/10 backdrop-blur-md p-3 rounded-xl border border-white/20">
                            ...
                            <a href="{{ route('face-attendance.index') }}" class="px-3 py-1.5 bg-white text-blue-600 rounded-lg font-semibold text-xs shadow-lg hover:bg-gray-50 transition">Scan</a>
                        </div>
                     @endif
                </div>
        ...
        <div class="max-w-[1400px] mx-auto px-2 sm:px-4 -mt-12 md:mt-8 md:grid md:grid-cols-12 md:gap-8 relative z-10">
            ...
            <div class="md:col-span-8 space-y-6">
                ...
                {{-- Deadline Countdown Widget - Always Show --}}
                @if($nextDeadline)
                    <div class="mobile-card rounded-2xl p-4 sm:p-5 animate-slide-up delay-100">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div id="countdown-icon" class="w-12 h-12 rounded-xl 
                                    @if($deadlineType === 'waiting') bg-gradient-to-br from-gray-500 to-gray-600
                                    @elseif($deadlineType === 'clock_in') bg-gradient-to-br from-red-500 to-orange-500
                                    @elseif($deadlineType === 'clock_out') bg-gradient-to-br from-blue-500 to-indigo-500
                                    @else bg-gradient-to-br from-purple-500 to-indigo-500
                                    @endif
                                ...
                                <div>
                                    ...
                                    <p id="countdown-subtitle" class="text-xs text-gray-500 dark:text-gray-400">
                                        @if($deadlineType === 'waiting') Buka: {{ $nextDeadline->format('H:i') }} WIB
                                        @elseif($deadlineType === 'clock_in') Deadline: {{ $nextDeadline->format('H:i') }} WIB
                                        @elseif($deadlineType === 'clock_out') Waktu Keluar: {{ $nextDeadline->format('H:i') }} WIB
                                        @else Buka Besok: {{ $nextDeadline->format('H:i') }} WIB
                                        @endif
                                    </p>
                                </div>
                            </div>
                        ...
                        @if(!$attendanceToday && $canClockIn)
                            <a href="{{ route('face-attendance.index') }}" class="mt-3 w-full flex items-center justify-center gap-2 bg-gradient-to-r from-red-500 to-orange-500 hover:from-red-600 hover:to-orange-600 text-white font-bold py-2.5 px-4 rounded-xl shadow-lg transition-all group">
                        ...
                        @elseif(!$attendanceToday && !$canClockIn)
                        ...
                        @elseif($attendanceToday && !$attendanceToday->time_out && $deadlineType === 'clock_out')
                            <a href="{{ route('face-attendance.index') }}" class="mt-3 w-full flex items-center justify-center gap-2 bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white font-bold py-2.5 px-4 rounded-xl shadow-lg transition-all group">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="flex gap-4 mt-1">
                                    <div>
                                        ...
                                        <p class="text-sm font-bold text-gray-800 dark:text-white">{{ $attendanceToday->time_in ? \Carbon\Carbon::parse($attendanceToday->time_in)->format('H:i') : '-' }}</p>
                                    </div>
                                    <div class="w-px bg-gray-200 dark:bg-gray-700"></div>
                                    <div>
                                        <span class="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold">Keluar</span>
                                        <p class="text-sm font-bold text-gray-800 dark:text-white">{{ $attendanceToday->time_out ? \Carbon\Carbon::parse($attendanceToday->time_out)->format('H:i') : '-' }}</p>
                                    </div>
                                    @if($attendanceToday->time_out)
                                        <div class="w-px bg-gray-200 dark:bg-gray-700"></div>
                                        <div>
                                            <span class="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold">Durasi</span>
                                            <p class="text-sm font-bold text-gray-800 dark:text-white">{{ $attendanceToday->duration ?? '-' }}</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
            ...
            <div class="md:col-span-4 space-y-6 mt-6 md:mt-0">
                ...
                <div class="animate-slide-up delay-300">
                    ...
                    <div class="grid grid-cols-4 md:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
                        ...
                        <a href="{{ route('academic-calendar') }}" class="flex flex-col items-center gap-2 group">
                ...
                <div class="animate-slide-up delay-400">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('attendance-history') }}" class="text-xs font-semibold text-blue-600 hover:text-blue-700">Lihat Semua</a>
                    </div>
                    <div class="space-y-3">
                        @forelse($history as $record)
                            <div class="group relative bg-white dark:bg-gray-800 p-3 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex items-center justify-between hover:shadow-md transition-all duration-300">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 class="font-bold text-gray-800 dark:text-white text-xs">Presensi {{ $record->time_out ? 'Selesai' : 'Harian' }}</h4>
                                        <p class="text-[10px] text-gray-500 mt-0.5">{{ \Carbon\Carbon::parse($record->date)->translatedFormat('l, d F Y') }}</p>
                                    </div>
                                </div>
                                <div class="flex flex-col items-end gap-1">
                                    <div class="flex items-center gap-2">
                                        <div class="flex flex-col items-end">
                                            ...
                                            <span class="text-[11px] font-bold text-gray-800 dark:text-white">{{ $record->time_in ? \Carbon\Carbon::parse($record->time_in)->format('H:i') : '-' }}</span>
                                        </div>
                                        @if($record->time_out)
                                            ...
                                            <div class="flex flex-col items-end">
                                                ...
                                                <span class="text-[11px] font-bold text-gray-800 dark:text-white">{{ \Carbon\Carbon::parse($record->time_out)->format('H:i') }}</span>
                                            </div>
                                        @endif
                                    </div>
                                    <span class="text-[9px] uppercase font-bold {{ $record->status == 'present' ? 'text-green-500 bg-green-50' : 'text-amber-500 bg-amber-50' }} dark:bg-opacity-10 px-1.5 py-0.5 rounded-full">
    ...
    <script>
        ...
        const deadlineTime = new Date('{{ $nextDeadline->format('Y-m-d H:i:s') }}').getTime();
        const deadlineType = '{{ $deadlineType }}'; // clock_in, clock_out, next_day, waiting
        const startOfDay = new Date('{{ $nextDeadline->copy()->startOfDay()->format('Y-m-d H:i:s') }}').getTime();
