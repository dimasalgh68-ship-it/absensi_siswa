<x-teacher-layout>
    <x-slot name="header">
        <div class="d-flex justify-content-between align-items-center">
            <h1 class="h3 mb-0 text-gray-800 font-weight-bold">Edit Jadwal</h1>
        ...
        <div class="col-lg-8">
            <div class="card border-0 shadow-premium rounded-2xl overflow-hidden mb-4">
                <div class="card-header bg-white py-3 border-bottom-light">
                    <h6 class="m-0 font-weight-bold text-primary"><i class="fas fa-edit mr-2"></i>Formulir Edit Jadwal</h6>
                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control rounded-xl" id="education_id" name="education_id" required>
                                        ...
                                        @foreach($educations as $edu)
                                            <option value="{{ $edu->id }}" {{ old('education_id', $schedule->education_id) == $edu->id ? 'selected' : '' }}>{{ $edu->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input type="time" class="form-control rounded-xl" id="start_time" name="start_time" value="{{ old('start_time', $schedule->start_time) }}" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="font-weight-600 text-slate-700" for="end_time">Jam Selesai</label>
                                    <input type="time" class="form-control rounded-xl" id="end_time" name="end_time" value="{{ old('end_time', $schedule->end_time) }}" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label class="font-weight-600 text-slate-700" for="room">Ruangan / Ruang Kelas</label>
                                    <input type="text" class="form-control rounded-xl" id="room" name="room" value="{{ old('room', $schedule->room) }}" placeholder="Contoh: R. Laboratorium">
                                </div>
                            </div>
                        </div>
