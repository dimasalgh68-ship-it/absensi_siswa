<x-form-section submit="updateProfileInformation">
  ...
  <x-slot name="description">
    Perbarui informasi profil dan data pribadi Anda sebagai guru.
  </x-slot>
  ...
  <x-slot name="form">
    ...
    <div class="col-span-6 flex flex-col sm:flex-row gap-3 sm:col-span-4">
      ...
      <div class="w-full">
        ...
        <x-input id="birth_place" type="text" class="mt-1 block w-full" wire:model="form.birth_place" placeholder="Kota kelahiran" />
        <x-input-error for="form.birth_place" class="mt-2" />
      </div>

      <!-- Birth Date -->
      <div class="w-full">
        <x-label for="birth_date" value="Tanggal Lahir" />
        <x-input id="birth_date" type="date" class="mt-1 block w-full" wire:model="form.birth_date" />
    ...
    <div class="col-span-6 flex flex-col sm:flex-row gap-3 sm:col-span-4">
      ...
      <div class="w-full">
        ...
        <x-input id="phone" type="text" class="mt-1 block w-full" wire:model="form.phone" required placeholder="08xxxxxxxxxx" />
      ...
      <div class="w-full">
        ...
        <x-select id="gender" class="mt-1 block w-full" wire:model="form.gender" required>
    ...
    <div class="col-span-6 sm:col-span-4">
      ...
      <x-textarea id="address" class="mt-1 block w-full" wire:model="form.address" required rows="3" placeholder="Jalan, RT/RW, Kelurahan, Kecamatan" />
    ...
    <div class="col-span-6 sm:col-span-4">
      ...
      <x-input id="city" type="text" class="mt-1 block w-full" wire:model="form.city" required placeholder="Nama kota/kabupaten" />
    ...
    <div class="col-span-6 sm:col-span-4">
      ...
      <x-input id="email" type="email" class="mt-1 block w-full" wire:model="form.email" required
    ...
    <div class="col-span-6 sm:col-span-4">
      <hr class="border-gray-200 dark:border-gray-700">
      <h3 class="mt-4 text-sm font-semibold text-gray-700 dark:text-gray-300">Informasi Guru</h3>
    </div>

    <!-- Kelas yang Diampu -->
    <div class="col-span-6 sm:col-span-4">
      <x-label for="education" value="Kelas yang Diampu" />
      <x-select id="education" class="mt-1 block w-full" wire:model="form.education_id">
        <option value="">Pilih Kelas</option>
        @foreach (App\Models\Education::all() as $education)
          <option value="{{ $education->id }}" {{ $education->id == ($form->education_id ?? null) ? 'selected' : '' }}>
            {{ $education->name }}
          </option>
        @endforeach
      </x-select>
      <x-input-error for="form.education_id" class="mt-2" />
      <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Pilih kelas yang Anda ajarkan</p>
    </div>

    <!-- Jurusan -->
    <div class="col-span-6 sm:col-span-4">
      <x-label for="job_title" value="Jurusan" />
      <x-select id="job_title" class="mt-1 block w-full" wire:model="form.job_title_id">
        <option value="">Pilih Jurusan</option>
        @foreach (App\Models\JobTitle::all() as $jobTitle)
          <option value="{{ $jobTitle->id }}" {{ $jobTitle->id == ($form->job_title_id ?? null) ? 'selected' : '' }}>
            {{ $jobTitle->name }}
          </option>
        @endforeach
      </x-select>
      <x-input-error for="form.job_title_id" class="mt-2" />
      <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Pilih jurusan yang Anda ajarkan</p>
    </div>
  </x-slot>
